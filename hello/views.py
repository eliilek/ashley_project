...
def trial(request):
    ...
    for i in range(symbol_set.block_size / len(single_sets)):
        for current_set in single_sets:
            ...
            temp_dict["left"]["image"] = cloudinary.CloudinaryImage(temp_dict["left"]["image"].public_id).build_url(width=50)
            temp_dict["center"] = model_to_dict(options_list[1])
            temp_dict["center"]["image"] = cloudinary.CloudinaryImage(temp_dict["center"]["image"].public_id).build_url(width=50)
            temp_dict["right"] = model_to_dict(options_list[2])
            temp_dict["right"]["image"] = cloudinary.CloudinaryImage(temp_dict["right"]["image"].public_id).build_url(width=50)
    ...
    for i in range(len(trial)):
        ...
        trial[i]["stimulus"] = cloudinary.CloudinaryImage(Symbol.objects.get(pk=trial[i]["stimulus"]).image.public_id).build_url(width=50)
